...
public class FogDevice extends PowerDatacenter {
    ...
    protected void sendPeriodicTuple(SimEvent ev) {
        ...
        for (int i = 0; i < ((edge.getDirection() == Tuple.UP) ? instanceCount : 1); i++) {
        	...
        	tuple.setActualTupleId(tupleId);
        	System.out.println("SENDING → TupleID: " + tupleId + " from " + edge.getSource() + " to " + edge.getDestination() + " at time: " + CloudSim.clock());

        	// Find the correct loop ID
        	for (AppLoop loop : applicationMap.get(module.getAppId()).getLoops()) {
        	    if (loop.hasEdge(edge.getSource(), edge.getDestination())) {
        	        int loopId = loop.getLoopId();

//        	        if (!TimeKeeper.getInstance().getLoopIdToTupleIds().containsKey(loopId)) {
//        	            TimeKeeper.getInstance().getLoopIdToTupleIds().put(loopId, new ArrayList<>());
//        	        }
        	        TimeKeeper.getInstance().getLoopIdToTupleIds()
                    .computeIfAbsent(loopId, k -> new ArrayList<>())
                    .add(tupleId);

                TimeKeeper.getInstance().getEmitTimes().put(tupleId, CloudSim.clock());
                break; // Stop once the first matching loop is found
        	    }
        	}
    ...
    protected void processTupleArrival(SimEvent ev) {
        ...
        System.out.println("🟢 Processing Tuple: " + tuple.getTupleType() + " at Device: " + getName());
    ...
    protected void updateTimingsOnReceipt(Tuple tuple) {
        ...
        for (AppLoop loop : loops) {
            if (loop.hasEdge(srcModule, destModule) && loop.isEndModule(destModule)) {
                ...
                if (startTime == null)
//                {
//                    System.out.println("⚠️ VM or Cloudlet is null for tuple: " + tuple.getTupleType());
//                    return;
//                }
                	break;
                ...
                System.out.println("RECEIVED → TupleID: " + tuple.getActualTupleId() + " from: " + srcModule + " to: " + destModule);
                System.out.println("StartTime: " + startTime + ", Now: " + CloudSim.clock() + ", Delay: " + delay);
                System.out.println("[Loop Delay] Tuple ID: " + tuple.getActualTupleId() +
                        " | Src: " + srcModule + " -> Dest: " + destModule +
                        " | Delay: " + delay);
                break;
