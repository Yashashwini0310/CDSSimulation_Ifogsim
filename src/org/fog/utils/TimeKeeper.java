...
public class TimeKeeper {
	...
	public void addCostCalcData(List<Integer> loopIds, String microserviceName, int deviceId, int tupleId) {
		...
		// double totalExecutionTime = tupleIdToExecutionTime.get(tupleId) +
		// costCalcData.get(loopid).get(microserviceName).get(deviceId).getSecond();
		// int totalRequestCount =
		// costCalcData.get(loopid).get(microserviceName).get(deviceId).getFirst() + 1;
		// costCalcData.get(loopid).get(microserviceName).put(deviceId, new
		// Pair<>(totalRequestCount, totalExecutionTime));
		// } else {
		// costCalcData.get(loopid).get(microserviceName).put(deviceId, new Pair<>(1,
		// tupleIdToExecutionTime.get(tupleId)));
		// }
		// } else {
	...
	public Map<String, List<Double>> getLoopDelays() {
		return loopDelays;
	}
